...
$RELION_CMD relion_motion_refine --i Refine3D/job057/run_data.star --f PostProcess/job059/postprocess.star --corr_mic MotionCorr/job005/corrected_micrographs.star --first_frame 1 --last_frame -1 --o ${RELION_OUTPUT_DIR}/Polish_t/ --min_p 10000 --eval_frac 0.5 --align_frac 0.5 --params3  --j $RELION_CPU_THREADS_PER_TASK
...
mpirun $RELION_MPI_FLAGS $RELION_CMD relion_motion_refine_mpi --i Refine3D/job057/run_data.star --f PostProcess/job059/postprocess.star --corr_mic MotionCorr/job005/corrected_micrographs.star --first_frame 1 --last_frame -1 --o ${RELION_OUTPUT_DIR}/Polish/ --params_file ${RELION_OUTPUT_DIR}/Polish_t/opt_params_all_groups.txt --combine_frames --bfac_minfreq 20 --bfac_maxfreq -1 --j $RELION_CPU_THREADS_PER_TASK
